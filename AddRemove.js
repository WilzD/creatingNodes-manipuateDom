...
function deleteItem(e) {
  ...
  //editing the li functions->delete the data from localstorage
  // ->put the data in the input box
  function editItem(e) {
    if (e.target.classList.contains('edit')) {
      var li = e.target.parentElement // because we want to delete the li and e(event) has li as parent element
      var editName=document.getElementById('item')
      var editDesc=document.getElementById('desc')
      editName.value=li.firstChild.textContent //we have to update the value  in input box
      editDesc.value=`${li.firstChild.nextSibling.textContent}`
      itemList.removeChild(li)

      var deleteObj = JSON.parse(localStorage.getItem(li.firstChild.textContent)) //finding the text content as object
      // var editName=document.getElementById('item')
      // var editName_text=document.createTextNode(JSON.parse())
      // editName.innerHTML=editName_text
      // console.log(editName)
      localStorage.removeItem(deleteObj.name)
